...
> **https://github.com/rk2348/team8**

---
# ZOOZOOM（ズーズーム） - 没入型デジタル動物園 VR体験コンテンツ

<!-- ▼ キービジュアル（タイトル画面やメインビジュアルの画像に差し替える） -->
![キービジュアル](https://raw.githubusercontent.com/rk2348/team8/main/docs/images/key_visual.png)
...
### ■ 世界観・コンテンツ
6. [主要バイオーム](#6-主要バイオーム)
7. [登場動物データ（Animal Data）](#7-登場動物データanimal-data)

### ■ ゲーム設計
8. [主要機能とコア・メカニクス](#8-主要機能とコアメカニクス)
9. [ゲームループ・ミッション・スコア設計](#9-ゲームループミッションスコア設計)
10. [動物AI・エコシステム設計](#10-動物aiエコシステム設計)
11. [オーディオデザイン方針](#11-オーディオデザイン方針)
12. [操作方法（Controls）](#12-操作方法controls)
13. [VR快適性・アクセシビリティ方針](#13-vr快適性アクセシビリティ方針)
14. [ユーザー体験フロー](#14-ユーザー体験フロー)

### ■ 技術仕様・開発環境
15. [動作環境・対応デバイス（Requirements）](#15-動作環境対応デバイスrequirements)
16. [パフォーマンス目標・技術仕様](#16-パフォーマンス目標技術仕様)
17. [開発環境](#17-開発環境)
18. [セットアップ・ビルド手順（Getting Started）](#18-セットアップビルド手順getting-started)
19. [プロジェクト構成（Project Structure）](#19-プロジェクト構成project-structure)
20. [実装スクリプト一覧（Script Reference）](#20-実装スクリプト一覧script-reference)
21. [ブランチ運用・コーディング規約](#21-ブランチ運用コーディング規約)

### ■ プロジェクト管理
22. [事前開発で行ったこと（Pre-Hackathon Preparation Log）](#22-事前開発で行ったことpre-hackathon-preparation-log)
23. [開発ロードマップ](#23-開発ロードマップ)
24. [開発スケジュール・マイルストーン](#24-開発スケジュールマイルストーン)
25. [チームメンバーと役割（Team & Roles）](#25-チームメンバーと役割team--roles)
26. [既知の不具合（Known Issues）](#26-既知の不具合known-issues)
27. [技術的な工夫と課題解決（Technical Highlights & Problem Solving）](#27-技術的な工夫と課題解決technical-highlights--problem-solving)
28. [生成AIの活用について（AI Usage）](#28-生成aiの活用についてai-usage)
29. [クレジット・使用素材（Credits）](#29-クレジット使用素材credits)
30. [参考文献（References）](#30-参考文献references)
31. [安全上の注意（Safety Notes）](#31-安全上の注意safety-notes)
32. [ライセンス（License）](#32-ライセンスlicense)

### ■ 付録
33. [用語集（Glossary）](#33-用語集glossary)
34. [今後の展望・拡張構想（Future Works）](#34-今後の展望拡張構想future-works)
...
![体験レイヤーの概念図](https://raw.githubusercontent.com/rk2348/team8/main/docs/images/concept_diagram.png)
...
- **主な動物とその役割:**
  ...
  - **シマウマ:** 群れで行動し、捕食者からの警戒と高速逃走を行う被食者。ボイド的アルゴリズムによる大規模な群れの動きを見せる（詳細は第8.4節参照）。
...
![サバンナ・エリアのスクリーンショット](https://raw.githubusercontent.com/rk2348/team8/main/docs/images/biome_savanna.png)

---

## 7. 登場動物データ（Animal Data）

各動物の生息環境・食性・食物連鎖上の役割は、モデリング／AI実装／生存ミッション設計の共通仕様となる。実際のデータは、生態情報リサーチ担当（谷田・有谷）の調査結果に基づき随時更新・確定していく前提の作業用リファレンスである。生態情報の出典は「[30. 参考文献（References）](#30-参考文献references)」に記録する。
...
### 8.2. 「憑依」システム（Possession System / USP）
...
- **憑依前の演出シーケンス:** Aボタンを押した瞬間に即座に憑依するのではなく、プレイヤーのリグが対象の動物の周りを実際に周回しながら見下ろし、動物側の演出アニメーション・SE・視界内オブジェクト表示・軽いカメラ振動を経てから憑依が完了する、映画的な「導入演出」を挟むことで、視点が切り替わる瞬間の説得力を高めている（詳細は第20章 `AnimalViewSwitch`、第27章参照）。
...
- **エリアへの自動配置:** `AnimalAreaSpawner` により、指定した矩形エリア内へ動物種ごとに適切な頭数・群れ単位で自動配置される仕組みを構築済み（第10章・第20章参照）。

---

## 9. ゲームループ・ミッション・スコア設計
...
- **時間圧縮:** 現実の数分間をゲーム内の「1日」として体験する（第8章と連動）。
- **ミッション文言の動的表示:** ミッションパネルのTMProテキストは `PossessionController.SetMissionText()` により、狩りの進行状況（例：「シカが逃げ出した！追いかけろ」）に応じて実行時に書き換え可能（第20章・第27章参照）。
...
プレイヤーが憑依していないNPC動物の振る舞いと、バイオーム内の生態系の設計方針。Hackit2026期間中に基本設計を実装済みで、以下は実際に組み込んだ仕様を反映している（詳細は第20章のスクリプト一覧を参照）。
...
- 遠景の個体に対するAI処理の簡略化（AI LOD、第27章参照）による負荷軽減。
...
- **素材管理:** 外部サイトから取得した音源は、ダウンロード時点で「[29. クレジット・使用素材（Credits）](#29-クレジット使用素材credits)」の表に必ず記録する。
...
![コントローラー操作図](https://raw.githubusercontent.com/rk2348/team8/main/docs/images/controls.png)
...
![ユーザー体験フロー図](https://raw.githubusercontent.com/rk2348/team8/main/docs/images/user_flow.png)

---

## 15. 動作環境・対応デバイス（Requirements）
...
## 18. セットアップ・ビルド手順（Getting Started）
...
- 第20章の命名・責務定義を正とし、当日の実装はこの一覧に沿って進める（変更が必要な場合はチームに共有のうえ本章・第20章を更新する）。

---

## 22. 事前開発で行ったこと（Pre-Hackathon Preparation Log）
...
| 生態データ調査 | サファリ・エリア対象動物（トラ・シマウマ等）の生息環境・食性・食物連鎖上の役割をリサーチし、出典を記録 | 第7章・第30章 |
| Notionタスク設計 | Notionインポート用のCSV形式タスクリスト（サブステップのチェックリスト付き）を作成 | 第23章・第24章 |
| スクリプト設計（設計のみ・実装は含まない） | 実装予定の約55本のC#スクリプトについて、命名規則・責務・フォルダ構成を事前に整理 | 第20章 |
| 開発環境の構築・確認 | GitHub Desktop ＋ Git LFS 連携の動作確認、Unity 2022.3 LTS ＋ Meta SDK のセットアップ手順の整理 | 第17章・第18章・第19章 |
| アートディレクション方針の策定 | ローポリゴン＋デフォルメスタイルの方向性、モデリング規約の骨子を策定（配色等の詳細は未確定） | 第6章 |
| 画面モックアップの作成 | タイトル・探索・サバイバル・リザルトなど主要画面、計3画面のモックアップを作成 | 第14章 |
| プロモーションデモの作成 | Three.js製のアニメーションHTMLプロモーションデモを作成し、コンセプトを視覚的に共有 | 第2章 |
| 開発ロードマップ・スケジュール策定 | Phase 1〜3の開発ロードマップ、3日間のタイムテーブル（マイルストーン）を策定 | 第23章・第24章 |
| 役割分担の確定 | 5名のチームメンバーの役割・担当ツールを確定 | 第25章 |
...
## 26. 既知の不具合（Known Issues）

把握済みの不具合と回避策の一覧。審査員のデモプレイ中に発生した場合も「認識済み」であることを示せるよう、発見次第ここに記録する。**解決済みのものも削除せず、開発過程の記録として残す**（第27章の課題解決ログとも対応）。
...
## 27. 技術的な工夫と課題解決（Technical Highlights & Problem Solving）

技術審査向けに、本プロジェクトで特に注力した実装・工夫と、開発過程で実際に直面した課題・解決策をまとめる。前半（27.1）は設計上のハイライト、後半（27.2）は「課題→原因→解決策」の形式で実装過程のログを詳細に記録している。
...
- **24〜30時間での開発マネジメント:** WBS・Notionによるタスク分解（第20章のスクリプト一覧を含む）と、5人の役割分担（Unity実装／Blenderモデリング／オーディオ・TA）の並列化により、短期間でのコア体験完成を目指す体制を構築。
- **Animatorの「Any State」統一設計:** 動物ごとにState数・遷移パターンが異なると管理コストが増大するため、全動物のAnimator ControllerをIdle／Walk／Eat／Run／Attack／Die（捕食者のみAttack）の状態群とし、個別State間の直接遷移ではなく「Any State→各State」への統一遷移で構成。どの状態にいてもTriggerが立った瞬間に確実に反応する設計とし、複数動物への横展開を容易にした（実装過程で発生した遷移方向の逆転ミスとその修正は27.2の#F6を参照）。
...
### 27.2 課題解決ログ（Challenge → Cause → Solution）

開発中に実際に直面した実装上の課題を、発生した順に「課題」「原因」「解決策」の形式で記録する。第26章「既知の不具合」の該当IDと対応している。
...
| F15 | 動物に接近してプロンプトが表示されている段階でも、Aボタンを押す前は自由に動けてしまう | 移動ロックの制御が演出シーケンス開始時（Aボタン押下後）にしか行われていなかった | `Update()`内で射程内にいる間は`playerController.enabled = false`にし、射程外に出たら（演出中でなければ）再度有効化する処理を追加。演出シーケンス自体が持つON/OFF制御とは`isPlayingPossessionIntro`フラグで排他制御している（#015） |
...
| 実装計画・スクリプト設計 | Claude | 第20章「実装スクリプト一覧」の命名・責務設計（クラス構成の事前整理。**コードそのものの生成ではない**） |
| コーディング支援 | Claude / GitHub Copilot | 開催期間中のC#スクリプトの雛形生成・機能追加・バグ修正のデバッグ補助（第27章の課題解決ログはこの過程の記録） |
| 一部モデリング | MeshyAI | 動物のモデリングおよびボーンの作成 |

---

## 29. クレジット・使用素材（Credits）
...
| フォント名 | 配布元 | URL | ライセンス |
| --- | --- | --- | --- |
| 源暎こぶり明朝 | CGbox | https://cgbox.jp/2023/07/27/unity-font/ | 利用規約に基づき使用／クレジット表記：不要 |
...
第7章の動物生態データの根拠となる出典一覧。教育用途を想定するコンテンツとして、生態情報の正確性を担保するために記録する（担当：谷田・有谷）。
...
本プロジェクトは **非公開（Private）プロジェクト** として扱う。

- 本プロジェクトの著作権はチームメンバー全員に帰属し、無断での外部公開・複製・改変・再配布・商用利用を禁止する。
- 本プロジェクトは Hackit2026 における開発・提出、および関係者内での学習・参考目的にのみ使用するものとし、第三者への提供は行わない。
- 使用するサードパーティ製アセット・SDK・フォント・音源等については、各提供元のライセンス条件を遵守すること（一覧は第29章を参照）。
- 本方針は現時点のチーム内合意によるものであり、将来的に公開・再配布等の方針変更を行う場合は、メンバー全員の合意のうえ本節を改定する。
...
| Boids（ボイド） | 結合・分離・整列の単純なルールから群れ行動を再現するアルゴリズム。本作では`HerdBehavior`が結合・分離を実装（第8.4節参照）。 |
| Meta Quest | Meta社が販売するスタンドアロン型VRヘッドセット。本作はQuest 3 / 3Sを主対象に、PCレスで動作する（第15章参照）。 |
| 反芻（はんすう） | 一度飲み込んだ食物を再び口の中に戻し、噛み直して消化する動物の行動。ウシなど一部の草食動物に見られる（第7章の登場動物データ参照）。 |
| リアルタイムライト | 事前計算（ベイク）せず、実行時にその都度計算される光源。動的な光の変化を表現できる一方、モバイルGPUでは負荷が高いため最小限に抑える方針（第16章参照）。 |
| ドローコール（Draw Call） | GPUに対して描画命令を1回発行すること。数が多いほど処理負荷が高くなるため、Quest単体動作では削減が重要な最適化指標となる（第16章参照）。 |
| 近傍検索用レジストリ | 同種・他種の個体をすばやく検索できるよう、動物の位置情報などを種族ごとに登録しておく仕組み。本作では`AnimalIdentity`が担う（第20章参照）。 |
| モックアップ | 実装に入る前に、画面のレイアウトや見た目を確認するための試作・デザイン案。本作ではタイトル・探索・憑依・サバイバル・リザルトなど計8画面分を事前作成（第22章参照）。 |
